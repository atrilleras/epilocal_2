...
source("replicar_nature/utils/geom_stepribbon.r")
datos1 <-loadWorkbook("replicar_nature/data/datos_nature.xlsx")
interventions  <- read.xlsx(datos1,"intervenciones") %>% as.data.frame()
...
make_three_panel_plot <- function(filename){
  ...
  print(sprintf("loading: %s",paste0("replicar_nature/results/",filename,'-stanfit.Rdata')))
...
make_plots <- function(data_country, covariates_country_long, 
  ...
  save_plot(filename = paste0("replicar_nature/figures/", country, "-three-pannel-", filename, ".png"),
            p, base_width = 14)
}
