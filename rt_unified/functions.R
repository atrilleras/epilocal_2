    ...
    
{
    ...
    rt$method_epiestim <- "R(t) EpiEstim_2_week"
    ...
    
{
    ...
    rt$method_epiestim <- "R(t) EpiEstim"
...
fechaConfiable <- as.Date(max(datos$fecha_de_notificacion),origin = "1900-01-01") - rezago
    ...
    days <- seq_along(inc_multiple$dates)[2:(length(inc_multiple$dates)-14)]
    RTo  <- estimate_R(inc_multiple, method = "parametric_si",
                       config = make_config(list(
                           mean_si = 6.48, std_si = 3.83,
                           t_start=days, t_end=days+14)))
    data_rt <- RTo$R
