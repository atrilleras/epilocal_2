...
process_covariates_2 <- function(countries, interventions, d, ifr.by.country,N2,serial.interval){
  ...
  for (Country in countries$Regions){
    ...
    interventions_region <- interventions[interventions$Country == Country, c(2,3,4)] # school, public, lockdown
    ...
    lockdown = region_intervention[,3]
    firstIntervention = 1*((school+ publicEvents+ lockdown ) >= 1)
    ## append data
    ...
    df_features = data.frame('school' = school,'publicEvents' = publicEvents,
                             'lockdown' = lockdown)
